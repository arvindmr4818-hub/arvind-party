      ...
      content: SingleChildScrollView(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...controller.packages.map((pkg) =>
            RechargePackageItem(
              package: pkg,
              isSelected: controller.selectedPackage.value?.id == pkg.id,
  ...
  Widget _payChip(String label, String value, bool selected) {
    return Padding(
      ...
      child: FilterChip(
        label: Text(label),
        selected: selected,
        onSelected: (_) => controller.selectedPaymentMethod.value = value,
    ...
    Get.dialog(AlertDialog(
      ...
      content: SingleChildScrollView(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Obx(() => Wrap(
            spacing: 8,
            children: controller.withdrawMethods.map((m) => FilterChip(
              label: Text(m.name),
              selected: controller.selectedWithdrawMethod.value == m.id,
              onSelected: (_) => controller.selectedWithdrawMethod.value = m.id,
            )).toList(),
          )),
          const SizedBox(height: 12),
          TextField(
          ...
          TextField(
            ...
            decoration: const InputDecoration(labelText: 'Account Details', border: OutlineInputBorder()),
          ),
          const SizedBox(height: 16),
          Obx(() => ElevatedButton(
